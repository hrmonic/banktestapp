...
- le routing et le bootstrap (`App.tsx`),
...
La suite s’appuie sur :

- un setup de tests partagé (`vitest.config.mjs` + `apps/starter/tests/setup.ts`),
- **MSW** (Mock Service Worker) pour mocker les appels HTTP dans les tests unitaires/intégration (voir `apps/starter/tests/api/apiClient.test.ts` pour un exemple avec `createApiClient`).
...
1. **Tests unitaires/components**
   - tester son contrat `module.tsx` (id/basePath/sidebarItems),
