...
- a shared test setup (`vitest.config.mjs` + `apps/starter/tests/setup.ts`),
- **MSW** (Mock Service Worker) to mock HTTP calls in unit/integration tests (see `apps/starter/tests/api/apiClient.test.ts` for an example using `createApiClient`).
...
1. **Unit/component tests**
   - test its `module.tsx` contract (id/basePath/sidebarItems),
