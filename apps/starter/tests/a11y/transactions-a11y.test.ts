...
test("transactions module n'a pas de violations a11y majeures selon axe-core", async () => {
  const TransactionsRoutes = TransactionsModule.routes;
        ...
        children: React.createElement(
          ...
          React.createElement(
            ...
            React.createElement(Route, {
              path: '/',
              element: React.createElement(TransactionsRoutes, null),
            })
          )
        ),
      })
  ...
  if (seriousViolations.length > 0) {
    console.error('[a11y] Violations transactions:', seriousViolations);
  }
